...
import { performSystemHealthCheck, displayHealthStatus } from '../../utils/apiHealthCheck';
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const healthErrors = displayHealthStatus(healthStatus);
        
        if (healthErrors && healthErrors.length > 0) {
          console.error('系统健康检查失败:', healthErrors);
          const errorMessage = healthErrors.map(e => e.message).join(', ');
          const suggestion = healthErrors[0].suggestion;
          ...
          if (fallbackData) {
            try {
              ...
              console.log('健康检查失败，已从本地缓存加载备用数据');
            } catch (parseErr) {
        ...
        const citiesWithRegions = await Promise.all(
          citiesData.map(async (city) => {
            try {
              // 确保城市有有效的ID
              if (!city.id) {
                console.warn(`城市 ${city.name || 'Unknown'} 缺少ID，跳过详情加载`);
                return {
                  ...city,
                  regions: [],
                  totalRegions: 0,
              ...
              console.log(`获取城市 ${city.name} 的详细信息，ID: ${city.id}`);
              // 获取城市详情，包含zones
  ...
  const handleCitySelect = async (city, skipHighlight = false, specificFSA = null) => {
    ...
    try {
      ...
      if (city.regions && city.regions.length > 0) {
        ...
        // 确保城市有有效的ID
        if (!city.id) {
          console.warn(`城市 ${city.name || 'Unknown'} 缺少ID，无法加载区域`);
          setCityRegions([]);
          setHighlightedFSAs([]);
          return;
        }
