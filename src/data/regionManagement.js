 ...
 * @returns {Promise<string|null>} 区域ID或null
 */
export const getRegionByFSA = async (fsaCode) => {
  // 导入所需函数
  const { getRegionFSAs } = await import('../utils/unifiedStorage.js');
  
  // 使用统一存储架构查找FSA属于哪个区域
  for (let regionId = 1; regionId <= 8; regionId++) {
    try {
      const fsaCodes = await getRegionFSAs(regionId.toString());
      if (Array.isArray(fsaCodes) && fsaCodes.includes(fsaCode)) {
        return regionId.toString();
      }
    } catch (error) {
      console.error(`检查区域 ${regionId} FSA数据失败:`, error);
    }
  }
  return null;
};
...
export const getRegionByFSALegacy = (fsaCode, regionConfigs) => {
  ...
  for (const [regionId, config] of Object.entries(regionConfigs)) {
    // 支持新的 fsaCodes 字段
    if (config.fsaCodes && Array.isArray(config.fsaCodes) && config.fsaCodes.includes(fsaCode)) {
      return regionId;
    }
    // 兼容旧的 fsa 字段
    if (config.fsa && Array.isArray(config.fsa) && config.fsa.includes(fsaCode)) {
      return regionId;
    }
  }
  return null;
...
export const batchAssignFSAsToRegion = (fsaCodes, toRegionId, regionConfigs) => {
  ...
  fsaCodes.forEach(fsaCode => {
    try {
      // 找到FSA当前所属区域
      const currentRegion = getRegionByFSALegacy(fsaCode, updatedConfigs);
