...
const DirectPostalCodeManager = ({ 
  ...
  const loadRegionPostalCodes = () => {
    ...
    try {
      // 使用兼容层获取邮编数据（同步调用）
      let codes = compatLayer.getRegionPostalCodes(selectedRegion);
  ...
  const saveRegionPostalCodes = (updatedCodes) => {
    try {
      // 使用兼容层保存邮编数据（同步调用）
      const success = compatLayer.setRegionPostalCodes(selectedRegion, updatedCodes);
   ...
   * 清空所有邮编
   */
  const handleClearAllPostalCodes = async () => {
    if (postalCodes.length === 0) {
      return;
    }

    // 显示确认对话框
    if (!window.confirm(`确定要清空所有 ${postalCodes.length} 个邮编吗？此操作不可撤销。`)) {
      return;
    }

    const success = saveRegionPostalCodes([]);
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={handleClearAllPostalCodes}
          disabled={postalCodes.length === 0}
          className="px-4 py-2 bg-red-500/20 hover:bg-red-500/30 border border-red-500/30 rounded-lg text-red-300 transition-colors flex items-center gap-2 text-sm disabled:opacity-50"
        >
