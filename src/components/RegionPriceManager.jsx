...
const RegionPriceManager = ({ 
  ...
  // 当选择变化或本地配置变化时，设置当前配置（仅用于本地存储路径）
  useEffect(() => {
  ...
  /**
   * 从其他区域复制价格（仅在非 API 模式下可用）
   */
  const handleCopyPricing = async (sourceRegionId) => {
    if (apiModeEnabled) return; // 只读模式不允许
  ...
  // 确定展示用的 weightRanges（API 优先，只读）
  const weightRangesToUse = apiModeEnabled
    ? (apiRanges || [])
    : (currentConfig?.weightRanges || []);

  if (!apiModeEnabled && !currentConfig) {
    return (
      <div className={`p-8 text-center ${className}`}>
  ...
  const priceStats = getPriceStats(weightRangesToUse);
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-500/30 rounded-lg">
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-medium text-white">价格配置概览</h4>
            <span className="text-sm text-blue-300">
              共 {weightRangesToUse.filter(r => r.isActive).length} 个活跃区间
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-40 overflow-y-auto">
            {weightRangesToUse
              ...
              .map((range, idx) => (
                <div key={range.id || idx} className="p-2 bg-gray-800/50 rounded border border-gray-700/50">
                  <div className="text-xs text-gray-400">{range.label}</div>
                  <div className="text-sm font-bold text-green-400">${range.price.toFixed(2)}</div>
                </div>
              ))}
          </div>
        </div>
      ...
      {/* 批量操作面板 */}
      <AnimatePresence>
        {showBatchOperations && !apiModeEnabled && (
          <motion.div
